...
fn upstream_repo_candidates(primary_repo_root: &Path) -> Vec<PathBuf> {
    ...
    if let Some(explicit) = std::env::var_os("KLA_CODE_UPSTREAM") {
        candidates.push(PathBuf::from(explicit));
    }

    for ancestor in primary_repo_root.ancestors().take(4) {
        candidates.push(ancestor.join("kla-code"));
    }

    candidates.push(primary_repo_root.join("reference-source").join("kla-code"));
    candidates.push(primary_repo_root.join("vendor").join("kla-code"));

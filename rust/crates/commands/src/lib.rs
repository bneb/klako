...
pub use plugins_cmd::{handle_plugins_slash_command, render_plugins_report, PluginsCommandResult};
pub use slash::{
    render_slash_command_help, resume_supported_slash_commands, slash_command_specs,
    suggest_slash_commands, SlashCommand, SlashCommandCategory, SlashCommandSpec,
};
...
mod tests {
    use super::discovery::{
        ...
        render_agents_report, render_skills_report, DefinitionSource, SkillOrigin, SkillRoot,
    };
    use super::git::{
    ...
    use super::plugins_cmd::{handle_plugins_slash_command, render_plugins_report};
    use super::slash::{
        render_slash_command_help, resume_supported_slash_commands, slash_command_specs,
    ...
    fn init_git_repo(label: &str) -> PathBuf {
        ...
        run_command(&root, "git", &["config", "user.name", "Kla Tests"]);
        run_command(&root, "git", &["config", "user.email", "kla@example.com"]);
    ...
    fn write_external_plugin(root: &Path, name: &str, version: &str) {
        fs::create_dir_all(root.join(".klako-plugin")).expect("manifest dir");
        fs::write(
            root.join(".klako-plugin").join("plugin.json"),
            format!(
    ...
    fn renders_help_from_shared_specs() {
        ...
        assert!(help.contains("available via kla --resume SESSION.json"));
    ...
    fn lists_skills_from_project_and_user_roots() {
        ...
        let project_commands = workspace.join(".kla").join("commands");
        ...
        let roots = vec![
            ...
            SkillRoot {
                source: DefinitionSource::ProjectKla,
        ...
        assert!(report.contains("Project (.kla):"));
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        let agents_help =
            super::discovery::handle_agents_slash_command(Some("help"), &cwd).expect("agents help");
        assert!(agents_help.contains("Usage            /agents"));
        assert!(agents_help.contains("Direct CLI       kla agents"));

        let agents_unexpected =
            super::discovery::handle_agents_slash_command(Some("show planner"), &cwd)
                .expect("agents usage");
        assert!(agents_unexpected.contains("Unexpected       show planner"));

        let skills_help = super::discovery::handle_skills_slash_command(Some("--help"), &cwd)
            .expect("skills help");
        assert!(skills_help.contains("Usage            /skills"));
        assert!(skills_help.contains("legacy /commands"));

        let skills_unexpected =
            super::discovery::handle_skills_slash_command(Some("show help"), &cwd)
                .expect("skills usage");

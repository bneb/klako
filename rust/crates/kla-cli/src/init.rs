...
const STARTER_KLA_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
const GITIGNORE_COMMENT: &str = "# Klako local artifacts";
const GITIGNORE_ENTRIES: [&str; 2] = [".kla/settings.local.json", ".kla/sessions/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let klako_dir = cwd.join(".kla");
    artifacts.push(InitArtifact {
        name: ".kla/",
        status: ensure_dir(&klako_dir)?,
    });

    let klako_json = cwd.join(".kla.json");
    artifacts.push(InitArtifact {
        name: ".kla.json",
        status: write_file_if_missing(&klako_json, STARTER_KLA_JSON)?,
    });
    ...
    let klako_md = cwd.join("KLA.md");
    let content = render_init_klako_md(cwd);
    artifacts.push(InitArtifact {
        name: "KLA.md",
        status: write_file_if_missing(&klako_md, &content)?,
    });
...
pub(crate) fn render_init_klako_md(cwd: &Path) -> String {
    let detection = detect_repo(cwd);
    let mut lines = vec![
        "# KLA.md".to_string(),
        String::new(),
        "This file provides guidance to Klako (kla.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.kla.json`; reserve `.kla/settings.local.json` for machine-local overrides.".to_string());
    lines.push("- Do not overwrite existing `KLA.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
#[cfg(test)]
mod tests {
    use super::{initialize_repo, render_init_klako_md};
    use std::fs;
    use std::path::Path;
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".kla/            created"));
        assert!(rendered.contains(".kla.json        created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("KLA.md           created"));
        assert!(root.join(".kla").is_dir());
        assert!(root.join(".kla.json").is_file());
        assert!(root.join("KLA.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".kla.json")).expect("read kla json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".kla/settings.local.json"));
        assert!(gitignore.contains(".kla/sessions/"));
        let klako_md = fs::read_to_string(root.join("KLA.md")).expect("read kla md");
        assert!(klako_md.contains("Languages: Rust."));
        assert!(klako_md.contains("cargo clippy --workspace --all-targets -- -D warnings"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join("KLA.md"), "custom guidance\n").expect("write existing kla md");
        fs::write(root.join(".gitignore"), ".kla/settings.local.json\n").expect("write gitignore");

        let first = initialize_repo(&root).expect("first init should succeed");
        assert!(first
            .render()
            .contains("KLA.md           skipped (already exists)"));
        let second = initialize_repo(&root).expect("second init should succeed");
        let second_rendered = second.render();
        assert!(second_rendered.contains(".kla/            skipped (already exists)"));
        assert!(second_rendered.contains(".kla.json        skipped (already exists)"));
        assert!(second_rendered.contains(".gitignore       skipped (already exists)"));
        assert!(second_rendered.contains("KLA.md           skipped (already exists)"));
        assert_eq!(
            fs::read_to_string(root.join("KLA.md")).expect("read existing kla md"),
            "custom guidance\n"
        );
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".kla/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".kla/sessions/").count(), 1);
    ...
    fn render_init_template_mentions_detected_python_and_nextjs_markers() {
        ...
        let rendered = render_init_klako_md(Path::new(&root));

...
impl RemoteSessionContext {
    ...
    pub fn from_env_map(env_map: &BTreeMap<String, String>) -> Self {
        Self {
            enabled: env_truthy(env_map.get("KLA_CODE_REMOTE")),
            session_id: env_map
                .get("KLA_CODE_REMOTE_SESSION_ID")
...
mod tests {
    ...
    fn remote_context_reads_env_state() {
        let env = BTreeMap::from([
            ("KLA_CODE_REMOTE".to_string(), "true".to_string()),
            (
                "KLA_CODE_REMOTE_SESSION_ID".to_string(),
    ...
    fn bootstrap_fails_open_when_token_or_session_is_missing() {
        let env = BTreeMap::from([
            ("KLA_CODE_REMOTE".to_string(), "1".to_string()),

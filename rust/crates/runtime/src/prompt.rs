...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            dir.join("KLA.md"),
            dir.join("KLAKO.local.md"),
            dir.join(".kla").join("KLA.md"),
...
fn render_project_context(project_context: &ProjectContext) -> String {
    ...
    if !project_context.instruction_files.is_empty() {
        bullets.push(format!(
            "Klako instruction files discovered: {}.",
...
fn render_instruction_files(files: &[ContextFile]) -> String {
    let mut sections = vec!["# Klako instructions".to_string()];
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No Klako settings files loaded.".to_string()
        ]));
        return lines.join("\n");
    }
...
fn get_simple_doing_tasks_section() -> String {
    let items = prepend_bullets(vec![
        "The user will primarily request you to perform software engineering tasks. When given an unclear instruction, consider it in the context of these tasks and the current working directory. For example, if asked to change 'methodName' to snake case, do not reply with just 'method_name', instead find the method in the code and modify the code.".to_string(),
        "In general, do not propose changes to code you haven't read. If a user asks about or wants you to modify a file, read it first. Understand existing code before suggesting modifications.".to_string(),
        "Keep changes tightly scoped to the request. Don't create helpers, utilities, or abstractions for one-time operations. Don't design for hypothetical future requirements. Three similar lines of code is better than a premature abstraction.".to_string(),
        "Do not create files unless they are required to complete the task. Prefer editing an existing file over creating a new one, to prevent file bloat.".to_string(),
...
fn get_actions_section() -> String {
    [
        ...
        "Carefully consider the reversibility and blast radius of actions. Generally you can freely take local, reversible actions like editing files or running tests.".to_string(),
        "For actions that are hard to reverse, affect shared systems beyond your local environment, or could otherwise be risky or destructive, check with the user before proceeding. The cost of pausing to confirm is low, while the cost of an unwanted action (lost work, unintended messages sent, deleted branches) can be very high.".to_string(),
        "Examples of risky actions:".to_string(),
        " - Destructive operations: deleting files/branches, dropping database tables, killing processes, rm -rf, overwriting uncommitted changes.".to_string(),
        " - Hard-to-reverse operations: force-pushing, git reset --hard, amending published commits, removing dependencies.".to_string(),
        " - Actions visible to others: pushing code, creating/closing PRs, posting to external services, modifying shared infrastructure.".to_string(),
        "When you encounter an obstacle, do not use destructive actions as a shortcut to simply make it go away. For instance, try to identify root causes and fix underlying issues rather than bypassing safety checks. Explore whether unexpected files represent the user's in-progress work before deleting them.".to_string(),
    ]
    .join("\n\n")
}
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".kla")).expect("nested kla dir");
        fs::write(root.join("KLA.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("KLAKO.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".kla")).expect("apps kla dir");
        fs::write(root.join("apps").join("KLA.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".kla").join("instructions.md"),
            "apps dot kla instructions",
        )
        .expect("write apps dot kla instructions");
        fs::write(nested.join(".kla").join("KLA.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".kla").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.kla/KLA.md")),
            "KLA.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("KLA.md"), "rules").expect("write instructions");
    ...
    #[test]
    fn load_system_prompt_reads_klako_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".kla")).expect("kla dir");
        fs::write(root.join("KLA.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".kla").join("settings.json"),
        ...
        let original_klako_home = std::env::var("KLA_CONFIG_HOME").ok();
...
",
        ...
        if let Some(value) = original_klako_home {
            std::env::set_var("KLA_CONFIG_HOME", value);
        } else {
            std::env::remove_var("KLA_CONFIG_HOME");
        }
    ...
    #[test]
    fn renders_klako_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".kla")).expect("kla dir");
        fs::write(root.join("KLA.md"), "Project rules").expect("write KLA.md");
        fs::write(
            root.join(".kla").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        ...
        assert!(prompt.contains("# Klako instructions"));
        assert!(prompt.contains("Project rules"));
        assert!(prompt.contains("permissionMode"));
    ...
    #[test]
    fn discovers_dot_klako_instructions_markdown() {
        let root = temp_dir();
        let nested = root.join("apps").join("api");
        fs::create_dir_all(nested.join(".kla")).expect("nested kla dir");
        fs::write(
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".kla/instructions.md")));
        assert!(
    ...
    fn renders_instruction_file_metadata() {
        let rendered = render_instruction_files(&[ContextFile {
            path: PathBuf::from("/tmp/project/KLA.md"),
            content: "Project rules".to_string(),
        }]);
        assert!(rendered.contains("# Klako instructions"));

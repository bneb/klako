...
impl ApiError {
    ...
    pub fn is_retryable(&self) -> bool {
        match self {
            ...
            | Self::AllTiersExhausted { .. }
            | Self::Configuration(_)
            | Self::ProviderRefusal(_) => false,
        }
    }
}
...
impl Display for ApiError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::AllTiersExhausted { message } => {
                write!(f, "all inference tiers exhausted: {message}")
            }
            Self::Configuration(message) => write!(f, "configuration error: {message}"),
            Self::ProviderRefusal(message) => write!(f, "model refused the instruction: {message}"),
        }
    }
}

...
async fn send_message_posts_json_and_parses_response() {
    ...
    let body = concat!(
        ...
        "\"type\":\"message\",",
        "\"role\":\"assistant\",",
        "\"content\":[{\"type\":\"text\",\"text\":\"Hello from Klako\"}],",
    ...
    assert_eq!(
        ...
        vec![OutputContentBlock::Text {
            text: "Hello from Klako".to_string(),
        }]
    );
...
async fn provider_client_dispatches_api_requests() {
    ...
    let client = match client {
        ProviderClient::KlaApi(client) => {
            ProviderClient::KlaApi(client.with_base_url(server.base_url()))
        }
...
async fn live_stream_smoke_test() {
    ...
    let mut stream = client
        .stream_message(&MessageRequest {
            model: std::env::var("KLA_MODEL").unwrap_or_else(|_| "claude-sonnet-4-6".to_string()),
            max_tokens: 32,
